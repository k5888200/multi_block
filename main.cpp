...
set<long long> H[Nhash];
long long hnum[Nhash][256];
...
bool block_check(unsigned char* data, int size){
	...
	flag = 1;
	long long h[Nhash];
	for(int i=0;i<Nhash;i++) h[i] = 0;
	for(char *s = (char*)str_host; *s != '\r'; s++)
		for(int i=0;i<Nhash;i++)
			h[i] = h[i] * 1234567891 + hnum[i][*s];
	//puts("");
	for(;*str_host != '\r'; str_host++) printf("%c",*str_host);
	for(int i=0;i<Nhash;i++){
		if(H[i].find(h[i]) == H[i].end()) flag = 0;
	}
	if(flag){
		printf(" ----- block");
	}
...
int main(int argc, char **argv)
{
	...
	for(int i=0;i<Nhash;i++){
		while((hnum[i][0] = (long long)rand()) == 0);
		hnum[i][0] *= hnum[i][0];
		for(int j=1;j<256;j++) hnum[i][j] = hnum[i][j-1] * hnum[i][0];
	}
	...
	while(fgets(block_host, 300, in) != NULL){
		...
		for(char *s = block_host; *s && *s != '\n'; s++){
			...
			if(!flag) {continue;}
			for(int i=0;i<Nhash;i++) h[i] = h[i] * 1234567891 + hnum[i][*s];
		}
		for(int i=0;i<Nhash;i++) if(H[i].find(h[i]) == H[i].end()) H[i].insert(h[i]); 
	}
